...
export const getDoctorById = createAsyncThunk("getDoctorById", async (userId) => {
    const response = await fetch(`https://doctor-appointment-backend-3uugd79z1.vercel.app/api/doctor/get-doctor-info-by-user-id/${userId}`, {
        method: 'GET',
        headers: {
...
export const updateDoctorInfo = createAsyncThunk("updateDoctorInfo", async (doctor) => {
    const response = await fetch("https://doctor-appointment-backend-3uugd79z1.vercel.app/api/doctor/update-doctor-info", {
        method: 'PUT',
        headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${token}`
